<template>
  <table class="spreadsheet" ref="tableElementRef">
    <colgroup>
      <col width="50" />
    ...
    <tbody>
      ...
      >
        ...
        <td
          ...
          :ref="(el) => setCellElementRef(el as Element, { colIndex, rowIndex })"
...
function onKeydownArrowKey(e: KeyboardEvent) {
  ...
  switch (e.key) {
    ...
    case 'ArrowRight':
      targetCursor.value = {
        colIndex: e.ctrlKey ? lastColIndex.value : referenceCursor.value.rowIndex + 1,
        rowIndex: referenceCursor.value.rowIndex,
      }
      break;
    default:
